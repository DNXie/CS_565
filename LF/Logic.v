...
(* actually should return a function *)
...
(* hard part is to prove this lemma *)
Lemma rev_append_nil : forall T l1 l2, 
  @rev_append T l1 l2 = @rev_append T l1 [] ++ l2.
Proof.
  intros T. induction l1 as [|h' t'].
  - reflexivity.
  - simpl. rewrite IHt'. intros. (* strange! it doesn't work if immediately intros *)
    rewrite (IHt' (h'::l2)).
...
Proof.
  ...
  induction x.
  - simpl. reflexivity.
  - simpl. rewrite <- IHx. rewrite rev_append_nil.
    reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct (evenb n) eqn:E.
    + destruct IHn as [k IHn].
      ...
      { rewrite evenb_S. rewrite E. reflexivity. }
      rewrite H. exists k. rewrite IHn. reflexivity.
    + assert (H: evenb (S n) = true).
      { rewrite evenb_S.  rewrite E. reflexivity. }
      rewrite H. destruct IHn as [k IHn]. rewrite IHn.
...
Proof.
  ...
  - intro. split.
    ...
    + apply andb_true_elim2 in H. assumption.
  - intro. destruct H as [H1 H2]. subst. reflexivity.
Qed.
...
Proof.
  intros. split. 
  - intro. destruct b1. left. reflexivity. destruct b2. 
    right. reflexivity. inversion H.
  - intro. destruct H as [H|H].
    + subst. simpl. reflexivity.
    + subst. destruct b1; reflexivity. 
Qed.
(** [] *)
  ...
  - intros. induction l. 
    ...
    + simpl. simpl in H.  split. 
      * rewrite andb_commutative in H. apply andb_true_elim2 in H.
        assumption. 
      * apply IHl. apply andb_true_elim2 in H. assumption.
  - intros. induction l.
    + simpl. reflexivity.
...
Proof.
  ...
  - assumption.
  - unfold not in H1. exfalso. apply H0. exists x. assumption.
Qed.
(** [] *)
