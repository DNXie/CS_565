...
Proof.
  intros. unfold swap_pair. destruct p. simpl. 
  reflexivity.
Qed.
(** [] *)
...
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with
  | [] => []
  | hd::tl => if hd =? 0 then nonzeros tl else hd::(nonzeros tl)
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | [] => true
  | h::t => if leb 1 (count h s2) then subset t (remove_one h s2) else false
  end.
...
Proof.
  induction l1.
  - intros. simpl. reflexivity.
  - intros. simpl. destruct (n =? 0) eqn:E.
    + apply IHl1.
...
Proof.
  intros. destruct x. simpl. apply eqb_refl.
Qed.
(** [] *)
...
Proof.
  intros. unfold update. simpl. rewrite <- eqb_id_refl.
  reflexivity.
Qed.
(** [] *)
