...
Example some_term_is_stuck :
  exists t, stuck t.
Proof.
  exists (prd tru). unfold stuck. split.
  - unfold step_normal_form. unfold not. intros [t' H].
    inversion H. inversion H1.
  - unfold not. unfold value. intros [H | H]; inversion H.
Qed.
...
Proof.
  intros. unfold step_normal_form. unfold value in H. destruct H as [H|H].
  - inversion H; subst; unfold not; intros [t' contra]; inversion contra.
  - induction H.
    + unfold not. intros. destruct H as [t' contra]. inversion contra.
    + unfold not. intros. destruct H0 as [t' contra].
      unfold not in IHnvalue. apply IHnvalue. inversion contra; subst. exists t1'.
      assumption. 
Qed.
...
Proof with eauto.
  ...
  - inversion H0; subst... 
    assert (H3: step_normal_form (scc t1)).
    { apply value_is_nf. auto. }
    unfold step_normal_form in H3. exfalso. unfold not in H3.
    apply H3. exists t1'. assumption.
  - inversion H0; subst... 
    + inversion H. 
    + assert (H3: step_normal_form (scc y2)).
      { apply value_is_nf. auto. }
      unfold step_normal_form in H3. unfold not in H3.
  ...
  - inversion H0; subst... inversion H. 
    + assert (H3: step_normal_form (scc t0)).
      { apply value_is_nf. auto. }
      unfold step_normal_form in H3. unfold not in H3.
      exfalso. apply H3. exists t1'. assumption.
    + apply IHstep in H2. subst...
