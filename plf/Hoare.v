...
Proof.
  ...
  inversion HE. subst. (* st' becomes (X !-> aeval st a; st) *)
...
Proof.
  ...
  intros st st' Hc HP. unfold hoare_triple in Hhoare. apply (Hhoare st st').
  - assumption.
...
Proof.
  ...
  apply hoare_asgn.
  intros st H.
  unfold assn_sub. unfold t_update. simpl. reflexivity.
Qed.
...
Proof.
  ...
  apply hoare_asgn.
  intros st H. unfold assn_sub. unfold t_update. simpl. omega.
Qed.
...
Proof.
  intros P Q HP HQ.
  destruct HP as [y HP']. apply (HQ 42 y). assumption.
  (* eapply HQ. destruct HP as [y HP']. *)
...
Proof.
  ...
  - (* left part of seq *)
    ...
    intros st H. subst. unfold assn_sub. unfold t_update. simpl. reflexivity.*)
  intros a n. apply hoare_seq with (Q := fun st => st X = n).
  - apply hoare_skip.
  - apply hoare_consequence_pre with (P' := (fun st => st X = n)[X |-> a]).
    + apply hoare_asgn.
...
(*
  ...
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + intros st H. unfold assn_sub. simpl. unfold t_update. simpl.
      split; reflexivity.
*)
  apply hoare_seq with (Q := (fun st => st X = 1 /\ st Y = 2)[Y |-> 2]).
  - apply hoare_asgn.
  - apply hoare_consequence_pre
      with (P' := (fun st => st X = 1 /\ st Y = 2)[Y |-> 2][X |-> 1]).
    + apply hoare_asgn.
    + intros st H. unfold assn_sub, t_update. simpl. split; reflexivity.
Qed.
...
Proof.
  ...
  unfold swap_program.
  eapply hoare_seq.
  - eapply hoare_seq. eapply hoare_asgn. eapply hoare_asgn.
  - eapply hoare_consequence_pre.
    + eapply hoare_asgn.
    + unfold assn_sub, t_update. simpl. *)
  unfold swap_program.
  apply hoare_seq with (Q := (fun st => st Y <= st X)[Y|->Z][X|->Y]).
  apply hoare_seq with (Q := (fun st => st Y <= st X)[Y|->Z]).
  apply hoare_asgn.
  apply hoare_asgn.
  apply hoare_consequence_pre with
    (P' := (fun st => st Y <= st X)[Y|->Z][X|->Y][Z|->X]).
  apply hoare_asgn.
  intros s H. unfold assn_sub, t_update. simpl. assumption.
Qed.
...
Proof.
  ...
  apply hoare_if.
  - apply hoare_consequence_pre with (P':=(fun st => st X <= st Y)[Y|->2]).
    apply hoare_asgn. intros st [_ H]. unfold bassn in H. unfold assn_sub, t_update.
    simpl. simpl in H. apply eqb_eq in H. omega.
  - apply hoare_consequence_pre with (P':=(fun st => st X <= st Y)[Y|->X+1]).
    apply hoare_asgn. intros st [_ H]. unfold bassn in H. simpl in H. unfold 
    assn_sub, t_update. simpl. omega.
Qed.
...
Proof.
  apply hoare_if.
  - apply hoare_consequence_pre 
      with (P':=(fun st => st Y = st X + st Z)[Z|->Y-X]). apply hoare_asgn.
    unfold bassn, assn_sub, t_update. simpl. intros st [_ H].
    apply le_plus_minus. apply leb_complete in H. assumption.
  - apply hoare_consequence_pre
      with (P':=(fun st => st Y = st X + st Z)[Y|->X + Z]). apply hoare_asgn.
    unfold bassn, assn_sub, t_update. simpl. intros st [_ H]. reflexivity.
Qed.
...
Theorem hoare_if1 : forall P Q b c,
  ...
  {{ P }} (CIf1 b c) {{ Q }}.
Proof.
  intros. unfold hoare_triple. unfold hoare_triple in H.
  unfold hoare_triple in H0. intros. 
  inversion H1. subst.
  - apply (H st st'). assumption. split. assumption. unfold bassn. assumption.
  - apply (H0 st st'). subst. apply E_Skip. split. assumption. unfold not. 
    intro. unfold bassn in H8. subst. rewrite H7 in H8. discriminate H8.
Qed.
...
Proof.
  apply hoare_consequence_post 
    with (Q':=(fun st => st X <= 3 /\ ~(bassn (X <= 2) st))).
  apply hoare_while. 
  apply hoare_consequence_pre 
    with (P':=(fun st => st X <= 3)[X|->X+1]).
  apply hoare_asgn. unfold bassn, assn_sub, t_update. simpl. intros st [H1 H2].
  apply leb_complete in H2. omega.
  unfold bassn, assn_sub, t_update. simpl. intros st [Hle Hb].
  simpl in Hb. destruct ((st X) <=? 2) eqn:Heqle.
  + unfold not in Hb. exfalso. apply Hb. reflexivity.
  ...
  unfold bassn, assert_implies. intros st [Hle Hb].
    ...
    apply leb_iff_conv in Heqle. omega. Qed.*)
Qed.
...
Proof.
  ...
  remember (REPEAT c UNTIL b END) as wcom eqn:Hwcom.
  induction H0; inversion Hwcom; subst; clear Hwcom.
  - split. 
    + apply (H st st'); assumption.
    + unfold bassn. assumption.
  - apply IHceval2. reflexivity. apply (H st st'); assumption.
Qed.
...
Proof.
  ...
  exists (fun _ => True).
  split.
  - unfold hoare_triple. intros st r H1 H2. inversion H1. subst. exists st.
    split. reflexivity. apply H2.
  - unfold not. unfold hoare_triple. intro H.
    assert (H1: (fun _ => 0) =[ ASSERT BFalse ]=> RError).
    { apply E_AssertFalse. simpl. reflexivity. }
    apply H in H1.
    + destruct H1 as [st [contra Htrue]]. discriminate contra.
    + intuition.
Qed.
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} ASSUME b {{Q}}).
Proof.
  intros P b Q H. unfold hoare_triple in H. unfold hoare_triple.
  intros st r H1 H2. 
  apply (H st r).
  - inversion H1. subst. apply E_AssertTrue. assumption.
  - assumption.
Qed.
