...
Proof.
  ...
  - inversion HT; subst. exists x0, t0. reflexivity.
  - inversion HT.
  - inversion HT.
Qed.
...
Proof.
  ...
  - inversion Ht. subst. apply IHt1 in H2. 
    apply IHt2 in H4. destruct H2 as [H2|H2].
    + destruct H4 as [H4|H4].
      * inversion H2.
        ** right. exists ([x0:=t2]t). apply ST_AppAbs. assumption.
        ** subst. inversion Ht. inversion H3.
        ** subst. inversion Ht. inversion H3.
      * right. destruct H4 as [t2' H4]. exists (app t1 t2').
        apply ST_App2; assumption.
    + right. destruct H2 as [t1' H2]. exists (app t1' t2).
      apply ST_App1; assumption.
  - inversion Ht. subst. apply IHt1 in H3. apply IHt2 in H5.
    apply IHt3 in H6.
    destruct H3 as [H3|H3]. inversion H3; subst.
    + inversion Ht.  inversion H4. 
    + right. exists t2. apply ST_TestTru.
    + right. exists t3. apply ST_TestFls.
    + right. destruct H3 as [t1' H3].
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  intros. unfold closed. unfold not. intros. 
  apply (free_in_context x0 t T empty) in H0.
  - destruct H0 as [T' H0]. inversion H0.
  - assumption.
Qed.
...
Proof with eauto.
  ...
  - (* T_Var *)
    apply T_Var. rewrite <- H0. assumption. apply afi_var.
  - (* T_Abs *)
    apply T_Abs.
