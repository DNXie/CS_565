...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  (* let *)
  | tlet y t1 t2 => tlet y (subst x s t1) (if eqb_string x y then t2 else (subst x s t2))
  (* fix *)
...
Example typechecks :
  empty |- test \in Nat.
Proof. unfold test. eauto 15. (* FILL IN HERE *) Qed.
...
Example typechecks :
  empty |- test \in Nat.
Proof. unfold test. eauto 20. (* FILL IN HERE *) Qed.
...
Proof with eauto.
  ...
  - destruct IHHt1...
    + destruct IHHt2...
      right. destruct H0 as [t2' H0]. exists (pair t1 t2')...
    + right. destruct H as [t1' H]. exists (pair t1' t2)...
  - destruct IHHt...
    + inversion H; subst; try solve_by_invert.
      right. exists v1...
    + right. destruct H as [t' H]. exists (fst t')...
  - destruct IHHt...
    + inversion H; subst; try solve_by_invert.
      right. exists v2...
    + right. destruct H as [t' H]. exists (snd t')...
  (* let *)
  - destruct IHHt1... destruct H as [t1' H].
    right. exists (tlet x t1' t2)...
  (* fix *)
  - right. destruct IHHt...
    + inversion H; subst; try solve_by_invert.
      exists ([x:=(tfix (abs x T11 t12))]t12)...
