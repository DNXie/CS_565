...
Proof.
  ...
  - unfold not. unfold hoare_triple. intro H.
    ...
    { apply E_AssertError. simpl. reflexivity. }
    apply H in H1.
...
Proof.
  ...
  intros st r H1 H2. 
  apply (H st r).
  - inversion H1. subst. apply E_AssertNormal. assumption.
  - assumption.
Qed.
...
Theorem hoare_assert : forall Q (b:bexp),
  {{Q /\ b}} ASSERT b {{Q}}.
Proof.
  intros Q b. unfold hoare_triple. intros st r H1 [HQ Hb].
  inversion H1. subst.
  - exists st. split. reflexivity. assumption.
  - simpl in Hb. rewrite Hb in H0. discriminate H0.
Qed.

Theorem hoare_assume : forall (Q: state -> Prop) (b:bexp),
  {{b -> Q}}  ASSUME b {{Q}}.
Proof.
  intros Q b. unfold hoare_triple. intros st r H1 H2.
  inversion H1. subst. exists st. split.
  - reflexivity.
  - apply H2. simpl. assumption.
Qed.
